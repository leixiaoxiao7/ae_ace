...
# ✅ 不再 flatten，数据保持 (N, 28, 28)
# x_train_2s shape: (N, 28, 28)
# x_test_2s  shape: (N, 28, 28)
...
# ✅ Input 直接接受 (28, 28)，模型内部用 Flatten/Reshape 处理
...
for i in range(n):
    ...
    plt.imshow(x_test_2s[i])       # ✅ 直接 (28, 28)，无需 reshape
    plt.title("Original")
    plt.axis("off")

    # Display noisy
    ax = plt.subplot(10, 3, 3*i + 2)
    plt.imshow(x_test_noisy[i])    # ✅ 直接 (28, 28)，无需 reshape
    plt.title("Noisy")
    plt.axis("off")

    # Display denoised
    ax = plt.subplot(10, 3, 3*i + 3)
    plt.imshow(decoded_imgs[i])    # ✅ 直接 (28, 28)，无需 reshape
    plt.title("Denoised")
    plt.axis("off")
